...
// 2026 Cohort Member Roster admin page.
//
// Three workflows on one page:
//   1. Add a single member (email required; everything else optional).
//   2. Bulk-add via textarea (one email per line OR CSV with header).
//   3. Edit / delete existing members from the table below.
//
// Admin-gated. The page itself short-circuits non-admins; firestore.rules
// is the real gate.
...
const MembersAdminPage: React.FC = () => {
  ...
  async function onBulkImport() {
    ...
    try {
      ...
      const skipNote =
        result.skipped > 0 ? ` (${result.skipped} skipped: ${result.invalidEmails.slice(0, 3).join(', ')}${result.invalidEmails.length > 3 ? '…' : ''})` : '';
  ...
  return (
    <Layout>
      <section style={{ padding: '3rem 0 5rem', minHeight: '70vh', background: '#fafafa' }}>
        <div className="container" style={{ maxWidth: 1100 }}>
          <header style={{ marginBottom: '2rem' }}>
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '2rem', color: '#0d9488', marginBottom: '0.25rem' }}>
              2026 Cohort Members
            </h1>
            <p style={{ color: '#52525b' }}>
              {fetchState === 'idle' ? `${rows.length} member${rows.length === 1 ? '' : 's'} in the roster.` : ''}
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: '1.5rem', marginBottom: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
              ...
              {editingEmail && (
                <button
                  type="button"
                  onClick={resetForm}
                  style={{ padding: '0.55rem 1rem', background: '#fff', color: '#52525b', border: '1px solid #d4d4d8', borderRadius: 6, cursor: 'pointer' }}
                >
                  Cancel
                </button>
              )}
          ...
          {/* Bulk add */}
          <div style={{ background: '#fff', borderRadius: 12, padding: '1.5rem', marginBottom: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <h2 style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>Bulk add</h2>
          ...
          {/* Table */}
          <div style={{ background: '#fff', borderRadius: 12, padding: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', gap: '0.75rem', flexWrap: 'wrap' }}>
              <h2 style={{ fontSize: '1.1rem', margin: 0 }}>Members</h2>
              <input
                type="search"
                  ...
                  <tbody>
                    {filtered.map((r) => (
                      <tr key={r.email} style={{ borderBottom: '1px solid #f4f4f5' }}>
                        ...
                        <td style={{ padding: '0.5rem 0.6rem', color: '#52525b', maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={r.tier || ''}>
                          {r.tier || ''}
                        </td>
