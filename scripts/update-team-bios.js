 ...
 * Requires you to sign in as an admin so Firestore rules allow writes.
 *
 * Usage:  node scripts/update-team-bios.js <email> <password>
 *
 * Example:
 *   node scripts/update-team-bios.js admin@example.com mypassword123
 */
...
if (!email || !password) {
  console.error('Usage: node scripts/update-team-bios.js <admin-email> <password>');
  console.error('  You must sign in as an admin user so Firestore rules allow writes.');
  process.exit(1);
}
...
// ── Bio updates ─────────────────────────────────────────────────────
// NOTE: Thom Bond and Clara Moisello are intentionally excluded
const BIO_UPDATES = {
  'Antonio Espinoza': {
...
async function updateAllBios() {
  ...
  console.log('\ud83d\udcc4 Fetching all active team members from Firestore...\n');
  const teamRef = collection(db, 'teamMembers');
  const q = query(teamRef, where('isActive', '==', true));
  const snapshot = await getDocs(q);

  console.log(`   Found ${snapshot.size} active team members.\n`);
  ...
  for (const docSnap of snapshot.docs) {
    ...
    if (update) {
      ...
      const docRef = doc(teamRef, docSnap.id);
  ...
  const notFound = [];
  for (const name of Object.keys(BIO_UPDATES)) {
  ...
  console.log(`   Skipped: ${skipped} (no update needed / Thom & Clara excluded)`);
  if (notFound.length > 0) {
    console.log(`\n   \u26a0\ufe0f  Names NOT found in Firestore:`);
    notFound.forEach((n) => console.log(`      - ${n}`));
  }
  console.log(`${'─'.repeat(50)}\n`);
  console.log('\u2705 Done! Bios updated in Firestore.');
}
