...
const WeeklyAdminPage: React.FC = () => {
  ...
  return (
    <Layout>
      <section className="weekly-admin-page">
        <div className="container">
          <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <h1>Weekly Content — Admin</h1>
              <p style={{ color: '#666', margin: 0 }}>Manage the weekly lessons. All files served from Firebase Storage under admin-only rules.</p>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <Link to="/weekly" className="btn-secondary">View as admin</Link>
              <button className="btn-primary" onClick={() => setEditing({ ...emptyWeek, weekNumber: nextWeekNumber(weeks) })}>+ New week</button>
            </div>
          </header>

          {success && <div style={{ padding: '0.75rem 1rem', background: '#ecfdf5', color: '#065f46', borderRadius: 8, marginBottom: '1rem' }}>{success}</div>}
          {error && <div style={{ padding: '0.75rem 1rem', background: '#fef2f2', color: '#991b1b', borderRadius: 8, marginBottom: '1rem' }}>{error}</div>}
          ...
          {fetchState === 'loading' && <div>Loading weeks…</div>}
